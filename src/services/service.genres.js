...
export const getGenresDetails = async (data) => {
    return await axiosInstance.get(`${GET_GENRES}/${data}?key=${apiKEY}`).then(res => res.data);
};

export const getGenresGames = async (slug, pageParam) => {
    return await axiosInstance.get(`${GET_GAMES}?key=${apiKEY}&genres=${slug}&page=${pageParam}`).then(res => res.data);
};