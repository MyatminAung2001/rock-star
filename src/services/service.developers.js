...
export const getDevelopers = async (pageParam) => {
    return await axiosInstance
        .get(`${GET_DEVELOPERS}?key=${apiKEY}&page=${pageParam}&page_size=12`)
        .then((res) => res.data);
};

export const getDevelopersDetails = async (slug) => {
    return await axiosInstance
        .get(`${GET_DEVELOPERS}/${slug}?key=${apiKEY}`)
        .then((res) => res.data);
};
...
export const getDevelopersGames = async ({ slug, pageParam }) => {
    return await axiosInstance
        .get(
            `${GET_GAMES}?key=${apiKEY}&developers=${slug}&page=${pageParam}&page_size=12`
        )
        .then((res) => res.data);
};
